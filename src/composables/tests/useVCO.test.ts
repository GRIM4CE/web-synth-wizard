...
describe('useVCO', () => {
  it('creates an oscillator with the correct type', () => {
    ...
    const result = createOscillator({
      ...
      oscillatorSettings: { type: 'sawtooth', baseFrequency: 440, pulseWidth: 0.5, engine: 'oscillator', resonatorModel: 'string', damping: 0.5, structure: 0, brightness: 1, position: 0 },
      stepNote: 0,
  ...
  it('sets a positive frequency on the oscillator', () => {
    ...
    createOscillator({
      audioContext: mock as unknown as AudioContext,
      oscillatorSettings: { type: 'sine', baseFrequency: 440, pulseWidth: 0.5, engine: 'oscillator', resonatorModel: 'string', damping: 0.5, structure: 0, brightness: 1, position: 0 },
      stepNote: 0,
      selectedMusicalKey: ref<MusicalKey>('C'),
      selectedOctave: ref<1 | 2 | 3 | 4 | 5 | 6 | 7>(4),
      quantize: ref(false),
  ...
  it('produces higher frequency for higher stepNote', () => {
    ...
    const baseParams = {
      oscillatorSettings: { type: 'sine' as OscillatorType, baseFrequency: 440, pulseWidth: 0.5, engine: 'oscillator' as const, resonatorModel: 'string' as const, damping: 0.5, structure: 0, brightness: 1, position: 0 },
      selectedMusicalKey: ref<MusicalKey>('C'),
      selectedOctave: ref<1 | 2 | 3 | 4 | 5 | 6 | 7>(4),
      quantize: ref(false),
    }
